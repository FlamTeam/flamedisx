...
class NRSource(fd.BlockModelSource): # TODO -- ADD SKEW!
    ...
    def s1s2_acceptance(self, s1, s2, s1_min=0, s1_max=500, s2_min=400, s2_max=2e5): # 231208 AV adjusted s1_min from 20 --> 5 phd
    ...
    def pdf_for_nphne(self, x, y, x_mean, y_mean, x_std, y_std, x_skew, y_skew, anti_corr, spectrum):      
    ...
    def estimate_mu(self, **params):
        ...
        # Quanta Binning (previous upper lim ph:2000,e:800)
        Nph_edges = tf.cast(tf.linspace(30,5000,150), fd.float_type()) # shape (Nph+1) # to save computation time, I only did a rough integration over Nph and Ne
        Ne_edges = tf.cast(tf.linspace(10,1000,155), fd.float_type()) # shape (Ne+1)
