...
def calculate_density(temp, pressure):
    ...
    1.8801286589442915e6 * np.exp(-((temp - 4.0820251276172212e2) / 2.7863170223154846e1)**2) - \
...
def calculate_drift_velocity(drift_field, density, temp):
    ...
    [-1.5000, 37.021, -1.1430, 6.4590, -4.0337, 855.43, 5.4238],  # 157
    [-1.4939, 47.879, 0.12608, 8.9095, -1.3480, 1310.9, 2.7598],  # 163
    ...
    Temperatures = np.array([100., 120., 140., 155., 157., 163., 165., 167., 184., 200., 230.])
    ...
    if (temp == Ti):
        ...
        offset = (np.sqrt((Tf * (vf - vi) - Ti * (vf - vi) - 4.) * (vf - vi)) + \
        np.sqrt(Tf - Ti) * (vf + vi)) / (2. * np.sqrt(Tf - Ti))
        slope = -(np.sqrt(Tf - Ti) * np.sqrt((Tf * (vf - vi) - Ti * (vf - vi) - 4.) * (vf - vi)) - \
        (Tf + Ti) * (vf - vi)) / (2. * (vf - vi))
