...
@export
class SGMakePhotonsElectronsNR(fd.Block):
    is_ER = False
    ...
    def _compute(self,
        ...
        def compute_single_energy(args, approx=False):
            ...
            r_final = tf.where(tf.math.is_nan(p_final),
...
@export
class SGMakePhotonsElectronER(SGMakePhotonsElectronsNR):
    is_ER = True

    special_model_functions = tuple(
        [x for x in SGMakePhotonsElectronsNR.special_model_functions if (x != 'mean_yields' and x != 'yield_fano')] +
