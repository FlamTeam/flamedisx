...
@export
class vNRSource(fd_nest.nestERSource):
    """CEvNS background source from B8, HEP, Atmospheric and DSNB neutrinos.
    Reads in energy spectrum from .pkl file, generated with LZ's DMCalc.
    Normalise such that the spectrum predicts 123.74 events in 1 tonne year.
    """
    ...
    def __init__(self, *args, fid_mass=1., livetime=1., **kwargs):
        ...
        df_vER = pd.read_pickle(os.path.join(os.path.dirname(__file__), 'background_spectra/CEvNS_spectrum.pkl'))

        self.energies = tf.convert_to_tensor(df_vER['energy_keV'].values, dtype=fd.float_type())
        scale = fid_mass * livetime * 123.74
        self.rates_vs_energy = tf.convert_to_tensor(df_vER['spectrum_value_norm'].values * scale, dtype=fd.float_type())
